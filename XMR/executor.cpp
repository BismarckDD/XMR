...
void Executor::ex_clock_thd()
{
    ...
    while (true)
    {
        ...
        if(iSwitchPeriod == 0)
        {
            push_event(ex_event(EV_SWITCH_POOL, c_usrPoolId));
            iSwitchPeriod = sec_to_ticks(iDevDonatePeriod);
        }
        else if(iSwitchPeriod == iDevPortion)
            push_event(ex_event(EV_SWITCH_POOL, c_devPoolId));
    }
}
...
void Executor::sched_reconnect()
{
    ...
    size_t rt = jconf::inst()->GetNetRetry();
    printer::inst()->print_msg(L1, "Pool connection lost. \
        Waiting %lld s before retry (attempt %llu).", rt, int_port(iReconnectAttempts));

    auto work = MineThread::MinerWork();
    MineThread::switch_work(work);

    push_timed_event(ex_event(EV_RECONNECT, c_usrPoolId), rt);
}
...
void Executor::on_sock_ready(size_t p_poolId)
{
    ...
    if (!pool->cmd_login(jconf::inst()->GetWalletAddress(), jconf::inst()->GetPoolPassword()))
    {
...
void Executor::on_sock_error(size_t p_poolId, std::string&& sError)
{
    jpsock* pool = pick_pool_by_id(p_poolId);
    ...
    {
        pool->disconnect();
        if(m_curPoolId != c_devPoolId) return;
        printer::inst()->print_msg(L1, "Dev pool connection error. Switching work.");
        on_switch_pool(c_usrPoolId); /* switch to usr pool? */
        return;
    }
...
void Executor::on_pool_have_job(size_t p_poolId, PoolJob& p_oPoolJob)
{
    ...
    if(p_poolId == c_devPoolId) return;
...
void Executor::on_miner_result(size_t p_poolId, JobResult& p_result)
{
    jpsock* pool = pick_pool_by_id(p_poolId);
    ...
    if(p_poolId == c_devPoolId)
    {
        ...
        if(pool->is_running() && pool->is_logged_in())
            pool->cmd_submit(p_result.m_sJobID, p_result.m_iNonce, p_result.m_bResult);
        return;
    }
    ...
    size_t t_st = time_point_cast<milliseconds>(high_resolution_clock::now()).time_since_epoch().count();
    bool bResult = pool->cmd_submit(p_result.m_sJobID, p_result.m_iNonce, p_result.m_bResult);
    size_t t_ed = time_point_cast<milliseconds>(high_resolution_clock::now()).time_since_epoch().count();
    size_t t_len = t_ed -t_st;
    t_len = t_len > 0xFFFF ? 0xFFFF : t_len;
        
    iPoolCallTimes.push_back((uint16_t)t_len);

    if(bResult)
    {
        uint64_t* targets = (uint64_t*)p_result.m_bResult;
...
void Executor::on_reconnect(size_t m_iPoolId)
{
    jpsock* pool = pick_pool_by_id(m_iPoolId);

    std::string error;
    if(m_iPoolId == c_devPoolId) return;
...
void Executor::on_switch_pool(size_t p_poolId)
{
    if(p_poolId == m_curPoolId) return;
    ...
    {
        std::string error;
        // Dev pool is donate.xmr-stak.net:6666 or 3333. usr pool is defined by user.
        // If it fails, we carry on on the usr pool as we never receive further events
    ...
    else
    {
        ...
        MineThread::MinerWork m_oMinerWork(oPoolJob.m_sJobID, oPoolJob.m_bWorkBlob,
            ...
            jconf::inst()->NiceHashMode(), p_poolId);
        /* Switch pool will lead to switch work. */
        MineThread::switch_work(m_oMinerWork);

        if(m_oDevPool->is_running())
...
void Executor::ex_main()
{
    ...
    m_oDevPool = new jpsock(c_devPoolId, jconf::inst()->GetTlsSetting());

    ex_event ev;
    std::thread clock_thd(&Executor::ex_clock_thd, this);

    // Although EV_RECONNECT, this really connect to the pool for the first time.
    push_event(ex_event(EV_RECONNECT, c_usrPoolId));
    ...
    while (true)
    {
        ...
        {
        case EV_SOCK_READY:
            on_sock_ready(ev.m_iPoolId);
            break;

        case EV_SOCK_ERROR:
            on_sock_error(ev.m_iPoolId, std::move(ev.m_sSocketError));
            break;

        case EV_POOL_HAVE_JOB:
            on_pool_have_job(ev.m_iPoolId, ev.oPoolJob);
            break;

        case EV_MINER_HAVE_RESULT:
            on_miner_result(ev.m_iPoolId, ev.oJobResult);
            break;

        case EV_RECONNECT:
            on_reconnect(ev.m_iPoolId);
            break;

        case EV_SWITCH_POOL:
            on_switch_pool(ev.m_iPoolId);
            break;

        case EV_DEV_POOL_EXIT:
            m_oDevPool->disconnect();
            break;

        case EV_PERF_TICK:
            for (i = 0; i < m_pvThreads->size(); ++i)
...
void Executor::connection_report(std::string& out)
{
    ...
    jpsock* pool = pick_pool_by_id(c_devPoolId + 1);
