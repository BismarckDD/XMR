...
jconf* jconf::s_oInst = nullptr;

jconf::jconf()
{
    m_opq = new OpaquePrivate();
}
...
{
    if(!m_opq->configValues[aCpuThreadsConf]->IsArray())
        return false;

    if(id >= m_opq->configValues[aCpuThreadsConf]->Size())
        return false;

    const Value& oThdConf = m_opq->configValues[aCpuThreadsConf]->GetArray()[id];
...
{
    const char* opt = m_opq->configValues[sUseSlowMem]->GetString();
...
{
    return m_opq->configValues[bTlsMode]->GetBool();
}

bool jconf::TlsSecureAlgos()
{
    return m_opq->configValues[bTlsSecureAlgo]->GetBool();
}

const char* jconf::GetTlsFingerprint()
{
    return m_opq->configValues[sTlsFingerprint]->GetString();
}

const char* jconf::GetPoolAddress()
{
    return m_opq->configValues[sPoolAddr]->GetString();
}

const char* jconf::GetPoolPassword()
{
    return m_opq->configValues[sPoolPwd]->GetString();
}

const char* jconf::GetWalletAddress()
{
    return m_opq->configValues[sWalletAddr]->GetString();
}

bool jconf::PreferIpv4()
{
    return m_opq->configValues[bPreferIpv4]->GetBool();
}

size_t jconf::GetThreadCount()
{
    if(m_opq->configValues[aCpuThreadsConf]->IsArray())
        return m_opq->configValues[aCpuThreadsConf]->Size();
    else
        return 0;
}
...
{
    return !m_opq->configValues[aCpuThreadsConf]->IsArray();
}

uint64_t jconf::GetCallTimeout()
{
    return m_opq->configValues[iCallTimeout]->GetUint64();
}

uint64_t jconf::GetNetRetry()
{
    return m_opq->configValues[iNetRetry]->GetUint64();
}

uint64_t jconf::GetGiveUpLimit()
{
    return m_opq->configValues[iGiveUpLimit]->GetUint64();
}

uint64_t jconf::GetVerboseLevel()
{
    return m_opq->configValues[iVerboseLevel]->GetUint64();
}

uint64_t jconf::GetAutohashTime()
{
    return m_opq->configValues[iAutohashTime]->GetUint64();
}

uint16_t jconf::GetHttpdPort()
{
    return m_opq->configValues[iHttpdPort]->GetUint();
}

bool jconf::NiceHashMode()
{
    return m_opq->configValues[bNiceHashMode]->GetBool();
}

const char* jconf::GetOutputFile()
{
    return m_opq->configValues[sOutputFile]->GetString();
}
...
bool jconf::check_cpu_features()
{
    ...
    m_bHaveAes = (cpu_info[2] & AESNI_BIT) != 0;
    bHaveSSE2 = (cpu_info[3] & SSE2_BIT) != 0;

    return bHaveSSE2;
}

bool jconf::parse_config(const char* p_sFilename)
{
    FILE * pFile;
    char * buffer;
    size_t flen;
    ...
    pFile = fopen(p_sFilename, "rb");
    if (pFile == NULL)
    {
        printer::inst()->print_msg(L0, "Failed to open config file %s.", p_sFilename);
        return false;
    }
    ...
    {
        fclose(pFile);
        printer::inst()->print_msg(L0, "Oversized config file - %s.", p_sFilename);
        return false;
    }
    ...
    {
        fclose(pFile);
        printer::inst()->print_msg(L0, "File is empty or too short - %s.", p_sFilename);
        return false;
    }
    ...
    if(fread(buffer+1, flen, 1, pFile) != 1)
    {
        free(buffer);
        fclose(pFile);
        printer::inst()->print_msg(L0, "Read error while reading %s.", p_sFilename);
        return false;
    }
    fclose(pFile);
    ...
    m_opq->jsonDoc.Parse<kParseCommentsFlag|kParseTrailingCommasFlag>(buffer, flen+2);
    free(buffer);

    if(m_opq->jsonDoc.HasParseError())
    {
        printer::inst()->print_msg(L0, "JSON config parse error(offset %llu): %s",
            int_port(m_opq->jsonDoc.GetErrorOffset()), GetParseError_En(m_opq->jsonDoc.GetParseError()));
        return false;
    }
    ...
    for(size_t i = 0; i < iConfigCnt; i++)
    {
        ...
        m_opq->configValues[i] = GetObjectMember(m_opq->jsonDoc, oConfigValues[i].sName);

        if(m_opq->configValues[i] == nullptr)
        {
            printer::inst()->print_msg(L0, "Invalid config file. Missing value \"%s\".", oConfigValues[i].sName);
            return false;
        }

        if(!checkType(m_opq->configValues[i]->GetType(), oConfigValues[i].iType))
        {
    ...
    if(!m_opq->configValues[iCallTimeout]->IsUint64() ||
        !m_opq->configValues[iNetRetry]->IsUint64() ||
        !m_opq->configValues[iGiveUpLimit]->IsUint64())
    {
        printer::inst()->print_msg(L0,
    ...
    if(!m_opq->configValues[iVerboseLevel]->IsUint64() || !m_opq->configValues[iAutohashTime]->IsUint64())
    {
        printer::inst()->print_msg(L0,
    ...
    if(!m_opq->configValues[iHttpdPort]->IsUint() || m_opq->configValues[iHttpdPort]->GetUint() > 0xFFFF)
    {
        printer::inst()->print_msg(L0,
...
#ifdef CONF_NO_TLS
    if(m_opq->configValues[bTlsMode]->GetBool())
    {
        printer::inst()->print_msg(L0,
    ...
    printer::inst()->set_verbose_level(m_opq->configValues[iVerboseLevel]->GetUint64());
