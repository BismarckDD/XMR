...
xmr128i soft_aesenc(xmr128i in, xmr128i key)
{
    ...
    x0 = cvtsi128_si32(in);
    x1 = cvtsi128_si32(shuffle_epi32(in, 0x55));
    x2 = cvtsi128_si32(shuffle_epi32(in, 0xAA));
    x3 = cvtsi128_si32(shuffle_epi32(in, 0xFF));

    xmr128i out = set_epi32(
...
{
    uint32_t X1 = cvtsi128_si32(shuffle_epi32(key, 0x55));
    uint32_t X3 = cvtsi128_si32(shuffle_epi32(key, 0xFF));
    sub_word((uint8_t*)&X1);
    sub_word((uint8_t*)&X3);
    return set_epi32(_rotr(X3, 8) ^ rcon, X3, _rotr(X1, 8) ^ rcon, X1);
}
