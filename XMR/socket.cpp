...
plain_socket::plain_socket(jpsock* err_callback) : m_pCallback(err_callback)
{
    m_hSocket = INVALID_SOCKET;
    m_pSockAddr = nullptr;
}

bool plain_socket::setHostname(const char* sAddr)
{
    char sAddrMb[256];
    char *sTmp, *sPort;
    ...
    if (ln >= sizeof(sAddrMb))
        return m_pCallback->set_socket_error("CONNECT error: Pool address overflow.");
    ...
    if ((sPort = strchr(sAddrMb, ':')) == nullptr)
        return m_pCallback->set_socket_error("CONNECT error: Pool port number not specified, please use format <hostname>:<port>.");
    ...
    m_pAddrRoot = nullptr;
    int err;
    if ((err = getaddrinfo(sAddrMb, sPort, &hints, &m_pAddrRoot)) != 0)
        return m_pCallback->set_socket_error_strerr("CONNECT error: GetAddrInfo: ", err);

    addrinfo *ptr = m_pAddrRoot;
    ...
    if (ipv4 == nullptr && ipv6 == nullptr)
    {
        ...
        m_pAddrRoot = nullptr;
        return m_pCallback->set_socket_error("CONNECT error: I found some DNS records but no IPv4 or IPv6 addresses.");
    }
    ...
    m_hSocket = socket(m_pSockAddr->ai_family, m_pSockAddr->ai_socktype, m_pSockAddr->ai_protocol);

    if (m_hSocket == INVALID_SOCKET)
    {
        freeaddrinfo(m_pAddrRoot);
        m_pAddrRoot = nullptr;
        return m_pCallback->set_socket_error_strerr("CONNECT error: Socket creation failed ");
    }
...
{
    int ret = ::connect(m_hSocket, m_pSockAddr->ai_addr, (int)m_pSockAddr->ai_addrlen);

    freeaddrinfo(m_pAddrRoot);
    m_pAddrRoot = nullptr;

    if (ret != 0) /* Connect failed, return error info. */
        return m_pCallback->set_socket_error_strerr("CONNECT error: ");
    else
        return true;
}
...
{
    int ret = ::recv(m_hSocket, buf, len, 0);

    if(ret == 0)
        m_pCallback->set_socket_error("RECEIVE error: socket closed");
    if(ret == SOCKET_ERROR || ret < 0)
        m_pCallback->set_socket_error_strerr("RECEIVE error: ");
...
bool plain_socket::send(const char* buf)
{
    ...
    while (pos != slen)
    {
        int ret = ::send(m_hSocket, buf + pos, slen - pos, 0);
        if (ret == SOCKET_ERROR)
        {
            m_pCallback->set_socket_error_strerr("SEND error: ");
            return false;
        }
        else
            pos += ret;
...
bool tls_socket::setHostname(const char* sAddr)
{
    if(ctx == nullptr)
    {
        init_ctx();
...
bool tls_socket::connect()
{
    ...
    if(pCallback->m_iPoolId != Executor::c_devPoolId)
    {
